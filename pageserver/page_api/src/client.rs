...
impl Client {
    ...
    pub fn new(
        ...
        if let Some(compression) = compression {
            // PageService bounds compressed request decompression before handing identity frames
            // to Tonic, so established clients may continue using the selected encoding in both
            // directions.
            inner = inner
