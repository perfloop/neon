...
fn build_fixture() -> Fixture {
    ...
    let selected_key = Key::MIN.add(1024);
    let keyspace = KeySpace {
        ranges: vec![selected_key..selected_key.next()],
    };
    ...
    Fixture {
        ...
        expected: vec![
            (selected_key, Lsn(100), true),
            (selected_key, Lsn(120), false),
        ],
    }
}
...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    eprintln!("PERFLOOP_INDEX_NODE_READS={index_node_reads}");

    let mut group = c.benchmark_group("delta_layer_plan_reads_single");
    group.sample_size(10);
    group.warm_up_time(Duration::from_millis(100));
    group.measurement_time(Duration::from_millis(300));
    group.throughput(Throughput::Elements(index_node_reads as u64));
    group.bench_function("one_range_1", |b| {
