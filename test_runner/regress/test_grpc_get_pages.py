...
def run(
    ...
    command = [
        str(binary), "--endpoint", f"http://localhost:{env.pageserver.service_port.grpc}",
        "--tenant-id", str(env.initial_tenant), "--timeline-id", str(env.initial_timeline),
        "--read-lsn", str(lsn), "--spcnode", str(spcnode), "--dbnode", str(dbnode),
        "--relnode", str(relnode), "--frame-size", str(size), "--fallback-chunk-size", str(CAP),
        "--mode", mode,
    ]
...
def test_grpc_get_pages_at_cap_warm_mean(
    ...
    warmup = run(pg_bin, neon_binpath, env, lsn, relation, CAP, "raw")
    assert warmup["status"] == "ok" and warmup["response_pages"] == CAP
    elapsed = 0
    results = []
    ...
    assert all(result["status"] == "ok" and result["response_pages"] == CAP for result in results)
    emit("grpc_get_pages_at_cap_elapsed_ns", elapsed / repetitions)
    emit("grpc_get_pages_at_cap_returned_pages", CAP)
...
def test_grpc_get_pages_frame_boundaries(
    ...
    repetitions = int(os.environ.get("PERFLOOP_GRPC_GET_PAGE_OVER_CAP_REPETITIONS", "1"))
    assert repetitions > 0
    neon_env_builder.num_pageservers = 1
    neon_env_builder.pageserver_config_override = f"max_get_vectored_keys={CAP}"
    env, relation, lsn = make_relation(
        neon_env_builder, "perfloop_grpc_get_pages_boundaries", FRAME_CAP + 1, striped=True
    )
    ...
    filters = {"smgr_query_type": "get_page_at_lsn", "tenant_id": str(env.initial_tenant), "timeline_id": str(env.initial_timeline)}
    ...
    for _ in range(repetitions):
        started = perf_counter_ns()
        over_cap.append(run(pg_bin, neon_binpath, env, lsn, relation, FRAME_CAP + 1, "raw"))
        elapsed += perf_counter_ns() - started
    timers = metric(env, SMGR, filters) - before_timers
    vectored = metric(env, VECTORED, {"task_kind": "PageRequestHandler"}) - before_vectored
    if completed:
        expected = {
        ...
        assert all(result == expected for result in over_cap)
        assert timers == 0 and vectored == 0
    else:
        assert all(
            result["status"] == "ok"
            and result["response_pages"] == FRAME_CAP + 1
            and result["image_byte_mismatches"] == 0
            for result in over_cap
        )
        assert timers > 0 and vectored > 0
    emit("grpc_get_pages_over_cap_admission_elapsed_ns", elapsed / repetitions)
    emit("server_get_page_timer_starts_per_over_cap_frame", timers / repetitions)
    emit("server_get_vectored_calls_per_over_cap_frame", vectored / repetitions)
    emit("grpc_get_pages_over_cap_invalid_request", int(over_cap[0]["status"] == "invalid_request"))
...
def test_grpc_get_pages_late_chunk_preflight_and_error_boundary(
    ...
    neon_env_builder.pageserver_config_override = f"max_get_vectored_keys={CAP}"
    env, relation, lsn = make_relation(neon_env_builder, "perfloop_grpc_get_pages_late", FRAME_CAP)
