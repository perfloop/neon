...
def test_grpc_get_pages_late_chunk_preflight_and_error_boundary(
    ...
    # implementation completes the same frame directly. They must classify the
    # same way so a candidate cannot reach the direct branch only opportunistically.
    ...
    assert warmup["outcome"] == preflight["outcome"]

    # The test-only hook is after the fourth candidate chunk. The pre-image
    # lacks that hook and classifies its earlier oversized-frame failure. A
    # direct-completed preflight must therefore reach the injected final-chunk
    # error; only a legacy-fallback preflight may retain the legacy error branch.
    ...
    if preflight["outcome"] == "direct_completed":
        assert failure["outcome"] == "final_chunk_injected_error"
    else:
        assert preflight["outcome"] == "legacy_fallback_completed"
        assert failure["outcome"] == "legacy_internal"
