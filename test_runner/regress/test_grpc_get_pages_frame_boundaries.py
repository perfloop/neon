...
def test_grpc_get_pages_frame_boundaries(
    ...
    # A bounded implementation has one frame admission policy for every oversized
    # direct request; a mixed result would mask a size-specific partial implementation.
    assert direct_completed in (0, len(DIRECT_FRAME_SIZES))
    ...
    # The baseline has no direct oversized-frame implementation and therefore
    # routes this 129-block stale-parent frame to individually in-limit children.
    # Once direct 33/100-block frames complete, the same implementation must reject
    # the aggregate frame before splitting rather than retain that legacy branch.
    if direct_completed == len(DIRECT_FRAME_SIZES):
        assert over_cap["outcome"] == "invalid_request"
        assert over_cap["response_pages"] == 0
        assert timer_starts == 0
        assert vectored_calls == 0
    else:
        assert over_cap["outcome"] == "legacy_accepted"
